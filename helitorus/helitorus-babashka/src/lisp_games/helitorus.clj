...
;;
;; That example is the original and this file is a port of it, restructured
;; into a namespace with a -main and unattended run arguments. The geometry,
;; the painter ordering, the backface test, the lighting and the palette are
;; his and are unchanged. His own header credits a Scittle canvas demo one
;; step further back.
;;
...
(ns lisp-games.helitorus
  ...
  Run it with `bb helitorus`, or `bb helitorus 10` to quit after ten seconds.
  Drag to turn, wheel to zoom, LEFT/RIGHT change the winding count and UP/DOWN
  the resolution along the spine.

  This is the compute-heavy one. Every frame rebuilds the whole surface in
  Clojure, projects it and sorts the rings, so unlike Pac-Man the interpreter
  is doing real work between frames. The HUD reports compute and draw
  milliseconds separately, which is the number worth comparing between
  runtimes rather than guessing at.

  Everything reaches raylib as a scalar. babashka.ffi can pass structs by
  value, so the packed-uint Color is a choice: a four-byte all-integer struct
  rides in one register anyway, and packing saves building a map per call.
  The surface goes out through rlgl immediate mode, one batch per ring,
  because rlgl cannot flush inside an open rlBegin/rlEnd and the whole figure
  overflows the vertex buffer in one go."
  (:require [babashka.ffi :as ffi :refer [defcfn]]))

(ffi/load-system-library "raylib")

;; --- the raylib surface this needs -------------------------------------------
...
;; a screenshot taken mid-frame only sees what has been flushed, so the
;; unattended capture path forces the batch out before asking for the pixels
...
;; --- run state ---------------------------------------------------------------
;; The original read these from *command-line-args* at load time. Here -main
;; sets them, so the namespace can be loaded without a window (see `bb check`).
...
;; --- the loop ----------------------------------------------------------------
;; Per-run, not load-time, so `bb check` can load this namespace without
;; opening a window or starting a clock.
...
(defn -main
  ...
  bb helitorus                    run until the window is closed
  bb helitorus 10                 quit after ten seconds of frame time
  bb helitorus 3 out.png 76       quit after three, capture frame 76
  bb helitorus 3 out.png 76 400   the same, starting at resolution 400"
  [& args]
  (let [[secs shot-path shot-frame res] args
        deadline (some-> secs parse-long)
        shot-frame (or (some-> shot-frame parse-long) 20)]
    (when-let [n (some-> res parse-long)]
      (reset! NU (max 60 (min MAX-NU n))))
    (init-window W H "lisp-games - helitorus")
    (set-target-fps 120)
